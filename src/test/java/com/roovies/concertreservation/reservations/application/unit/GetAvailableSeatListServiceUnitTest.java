...
public class GetAvailableSeatListServiceUnitTest {
    ...
    @Mock
    private ReservationConcertGatewayPort reservationConcertGatewayPort;

    @Mock
    private ReservationVenueGatewayPort reservationVenueGatewayPort;
    ...
    void 콘서트나_일정이_유효하지_않으면_예외가_발생해야_한다() {
        ...
        given(reservationConcertGatewayPort.findConcertSchedule(query.concertId(), query.date()))
                .willThrow(NoSuchElementException.class);

        // when & then
        assertThatThrownBy(() -> getAvailableSeatListService.getAvailableSeatList(query))
    ...
    void 조회한_콘서트_일정의_잔여좌석이_0일_경우_좌석정보없이_매진_상태로_응답해야_한다() {
        ...
        ExternalConcertSchedule schedule = ExternalConcertSchedule.builder()
                .id(5L)
                .date(date)
                .availableSeats(0)
        ...
        // when
        GetAvailableSeatListResult result = getAvailableSeatListService.getAvailableSeatList(query);
    ...
    void 예약된_좌석이_없을_경우_모든_좌석이_예약_가능해야_한다() {
        ...
        ExternalVenue venue = ExternalVenue.builder()
                .id(10L)
                .name("인천 아시아드 주경기장")
                .totalSeats(3)
                .seats(List.of(
                        ExternalVenue.SeatItem.builder()
                                .id(1L)
                                .row(1)
                                .seatNumber(1)
                ...
                .build();
        given(reservationVenueGatewayPort.findVenueWithSeats(schedule.venueId()))
                .willReturn(venue);
