...
public class GetAvailableSeatsService implements GetAvailableSeatsUseCase {
    ...
    public GetAvailableSeatsResult execute(GetAvailableSeatsQuery query) {
        ...
        ConcertScheduleInfo schedule = concertQueryPort.getConcertScheduleInfo(query.concertId(), query.date());
        ...
        ConcertHallInfo concertHall = concertHallQueryPort.getConcertHallInfo(schedule.concertHallId());
        ...
        // 6. 예약가능한 좌석들만 추출
        List<GetAvailableSeatsResult.SeatItem> availableSeats = concertHall.seats().stream()
                .filter(seat -> !reservedSeatIds.contains(seat.id()))
                .map(seat -> GetAvailableSeatsResult.SeatItem.builder()
                        .seatId(seat.id())
                        .row(seat.row())
